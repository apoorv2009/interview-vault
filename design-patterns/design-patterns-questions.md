...
5. [Architecture Patterns](#4-architecture-patterns)
6. [Code Smells & Anti-Patterns](#5-code-smells--anti-patterns)
...
### Q17. [Topic: Architecture] [EPAM] What is CQRS?

CQRS = Command Query Responsibility Segregation. Split the model into two:
- **Commands** → write side — validates, enforces business rules, returns void/id
- **Queries** → read side — returns DTOs optimised for display, no business rules

```csharp
// COMMAND — write side — business rules enforced here
public record CreateEngagementCommand(string TenantId, string CompanyId,
    ActivityType Type, DateTime ScheduledAt) : IRequest<Guid>;
...
public class CreateEngagementHandler : IRequestHandler<CreateEngagementCommand, Guid>
{
    public async Task<Guid> Handle(CreateEngagementCommand cmd, CancellationToken ct)
    {
        ...
        var activity = EngagementActivity.Create(cmd.TenantId, cmd.CompanyId,
            cmd.Type, cmd.ScheduledAt);
...
// QUERY — read side — optimised SELECT, no domain model overhead
public record GetEngagementDashboardQuery(string TenantId) : IRequest<DashboardDto>;
...
**Benefits:** Write model enforces all business rules. Read model joins any tables. Read side can be independently scaled (read replica, Redis cache).

---

### Q18. [Topic: Architecture] [EPAM] What is the Outbox Pattern?

**Problem:** Save to DB AND publish to Service Bus — if one succeeds and other fails → data inconsistency.

**Solution:** Write event to outbox table in SAME transaction as the entity. Background worker publishes from outbox.

```csharp
// Step 1: Save entity + outbox message in ONE transaction
public async Task Handle(CreateEngagementCommand cmd, CancellationToken ct)
{
    ...
    _context.OutboxMessages.Add(new OutboxMessage  // same DbContext = same transaction ✅
    {
        ...
        Payload   = JsonSerializer.Serialize(new EngagementCreatedEvent(activity.Id))
    });
    await _context.SaveChangesAsync(ct); // ONE commit — both or neither ✅
}

// Step 2: Background worker publishes outbox messages
    ...
    protected override async Task ExecuteAsync(CancellationToken ct)
    {
        while (!ct.IsCancellationRequested)
        {
            var unpublished = await _context.OutboxMessages
                .Where(m => m.ProcessedAt == null).Take(100).ToListAsync(ct);
...
**Guarantees:** DB fails → outbox not written → bus not published ✅. Bus fails → outbox retried next poll ✅. App crash → outbox picked up on restart ✅.

---

### Q19. [Topic: Architecture] [EPAM] What is the Saga Pattern?

**Problem:** Operation spans multiple microservices — if one fails midway, need to undo what others already did.

```
Capital Access — Report Generation spans 4 services:
  1. Report Service:    create job record   ← done ✅
  2. Ownership Service: fetch portfolio     ← done ✅
  3. Targeting Service: fetch targets       ← FAILS ❌
  Without Saga: steps 1+2 have side effects dangling forever
```

```csharp
// Azure Durable Functions orchestrate the Saga
    ...
    [OrchestrationTrigger] IDurableOrchestrationContext context)
{
    ...
    try
    {
        ...
        var ownershipData = await context.CallActivityAsync<OwnershipData>("FetchOwnershipData", input.PortfolioId);
        completed.Add("FetchOwnershipData");
        var targetingData = await context.CallActivityAsync<TargetingData>("FetchTargetingData", input.Filters);
        completed.Add("FetchTargetingData");
        await context.CallActivityAsync("GenerateReport", new { jobId, ownershipData, targetingData });
    }
    catch
    {
        // Run compensating transactions in REVERSE ORDER for completed steps
        foreach (var step in Enumerable.Reverse(completed))
            await context.CallActivityAsync($"Compensate_{step}", input);
        // All side effects undone — system in clean state ✅
    }
}
```
...
```
CreateReportJob   → Compensate: Delete report job
FetchOwnershipData→ Compensate: Release ownership lock
FetchTargetingData→ FAILS → trigger Compensate_FetchOwnershipData + Compensate_CreateReportJob
```

---

### Q20. [Topic: Architecture] [EPAM] What is BFF (Backend for Frontend)?

**Problem:** One API serving both Angular SPA and mobile — response shape is a compromise for both. SPA needs aggregated data; mobile needs minimal data.

```
Without BFF:
  Angular SPA  ──→  Single API → giant response (SPA uses 80%, mobile 20%) ❌
  Mobile App   ──→  Same API   → downloads 5× more data than needed ❌

With BFF:
  Angular SPA  → Web BFF    → internal microservices
  Mobile App   → Mobile BFF → same internal microservices
```

```csharp
// Web BFF — aggregates in parallel, shapes exactly for Angular dashboard
[HttpGet("dashboard")]
public async Task<DashboardResponse> GetDashboard()
{
    // Calls 3 microservices IN PARALLEL — BFF orchestrates ✅
    var engagementsTask = _engagementClient.GetRecentAsync(TenantId);
    ...
    {
        Engagements = engagementsTask.Result.Select(DashboardEngagementDto.From),
        Metrics     = metricsTask.Result,
        Alerts      = alertsTask.Result.Take(5)
    };
    // Angular gets ONE call instead of three ✅
    // Response shaped exactly for the component — no transformation in Angular ✅
}
```

---

### Q21. [Topic: Architecture] [EPAM] What is the Circuit Breaker pattern? What are its three states?

```
CLOSED (normal):      Requests pass through. Failure count tracked.
                      5 failures → trip to OPEN.

OPEN (failing):       ALL requests fail immediately — no attempt made.
                      Downstream service gets recovery time.
                      After 30s timeout → move to HALF-OPEN.
...
```csharp
// Capital Access: protect against Cosmos DB outages
var circuitBreaker = Policy
    .Handle<HttpRequestException>()
    .CircuitBreakerAsync(
        exceptionsAllowedBeforeBreaking: 5,
        durationOfBreak: TimeSpan.FromSeconds(30),
        onBreak:    (ex, dur) => _logger.LogWarning("Circuit OPEN for {Dur}", dur),
        onReset:    ()        => _logger.LogInformation("Circuit CLOSED — recovered"),
        onHalfOpen: ()        => _logger.LogInformation("Circuit HALF-OPEN — probe"));

var policy = Policy.WrapAsync(circuitBreaker, retryPolicy);
var result = await policy.ExecuteAsync(() => _ownershipClient.GetHoldingsAsync(tenantId));
```

**Why it matters — cascade failure:**
```
Without Circuit Breaker:
  Ownership service slow → Targeting waits → threads exhausted
  → Targeting slows → Report service waits → ALL services down ❌

With Circuit Breaker:
  Ownership service slow → circuit OPENS → Targeting gets instant error
  → Targeting returns cached data — rest of platform unaffected ✅
```
...
### Q22. [Topic: Design Patterns] [EPAM] What are code smells? Name and explain the most common ones.
