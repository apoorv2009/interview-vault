...
| 43 | Caching | [You deployed a new version, but some users still see the old one. As a developer, how would you troubleshoot this?](#you-deployed-a-new-version-but-some-users-still-see-the-old-one-as-a-developer-how-would-you-troubleshoot-this) |
| 44 | Database | [If Stored Procedures can be faster, why do teams still prefer app-side queries?](#if-stored-procedures-can-be-faster-why-do-teams-still-prefer-app-side-queries) |
| 45 | Microservices | [Your API Gateway is running, all microservices pass health checks, yet every client request returns HTTP 503 Service Unavailable. What could be causing this, and how would you troubleshoot it?](#your-api-gateway-is-running-all-microservices-pass-health-checks-yet-every-client-request-returns-http-503-service-unavailable-what-could-be-causing-this-and-how-would-you-troubleshoot-it) |
| 46 | Rate Limiting | [Your API Gateway runs on multiple instances. How do you implement distributed rate limiting so a user can't bypass limits by hitting different instances?](#your-api-gateway-runs-on-multiple-instances-how-do-you-implement-distributed-rate-limiting-so-a-user-cant-bypass-limits-by-hitting-different-instances) |
| 47 | Data Structures | [How do you calculate mid in Binary Search without causing integer overflow?](#how-do-you-calculate-mid-in-binary-search-without-causing-integer-overflow) |
| 48 | DevOps | [Your AI-generated code passes every test, but could it still fail in production? As an engineer, what would you check before deploying it?](#your-ai-generated-code-passes-every-test-but-could-it-still-fail-in-production-as-an-engineer-what-would-you-check-before-deploying-it) |
| 49 | Scaling | [Black Friday starts in 2 days. Your site handles 100K users – 10 million are expected. You can't rewrite the architecture. What do you do before the traffic hits?](#black-friday-starts-in-2-days-your-site-handles-100k-users--10-million-are-expected-you-cant-rewrite-the-architecture-what-do-you-do-before-the-traffic-hits) |
| 50 | Git | [What will happen if two people push code to the same branch at the same time, one modified a file the other deleted the same file?](#what-will-happen-if-two-people-push-code-to-the-same-branch-at-the-same-time-one-modified-a-file-the-other-deleted-the-same-file) |
| 51 | Pagination | [How will you return 100,000 (1 lakh) users to the client efficiently?](#how-will-you-return-100000-1-lakh-users-to-the-client-efficiently) |
