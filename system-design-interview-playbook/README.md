...
- [You have 3 servers. Each runs the same midnight cron job. Now every email is sent 3 times. Fix it.](concurrency-distributed-cron-deduplication.md)

### Database
- [Your database has grown from 10 million to 1 billion records. Queries that took 50ms now take 10 seconds. How do you fix it, without replacing the database?](database-billion-records-query-optimization.md)
- [You DELETE a million rows, but the database size doesn't shrink. Where did the space go?](database-delete-space-not-freed.md)
- [Both UUID and auto-increment give unique IDs. So why can UUIDs make your database SLOWER?](database-uuid-vs-autoincrement.md)

### DevOps
- [What are the different deployment strategies and when to use them?](devops-deployment-strategies.md)
...
- [A microservice is very slow because of external API calls — how will you optimize it?](microservices-external-api-optimization.md)
...
- [Users ask in casual Hindi-English like 'kitna refund milega for cancelled order' — but your docs are in formal English. How do you handle code-mixed queries in retrieval?](rag-code-mixed-hinglish-queries.md)
