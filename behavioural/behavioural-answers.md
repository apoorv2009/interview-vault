...
We didn't rewrite the portfolio. I ran it as an incremental, Strangler Fig migration, because at that scale a big-bang rewrite means the entire modernization is unvalidated until one late, high-risk cutover — incremental validates the target architecture in small, reversible waves instead. Concretely: application and data-tier assessment across the whole portfolio using Azure Migrate to inventory dependencies and technical debt; portfolio analysis to classify each app by business value and migration complexity; dependency mapping so the wave sequence respected which apps actually depended on which, so nothing moved before its dependencies were ready. High-value, low-complexity apps went first — that's what proved the pattern and built stakeholder confidence early, before we touched anything genuinely hard.

The routing seam was the mechanism that made this safe: an API Management layer in front of both legacy and new endpoints, routing per-route rather than per-release, so a wave's cutover was a routing-rule change, not a redeploy — and trivially reversible if something regressed. On the front end, the AngularJS-to-Angular-18 migration followed the same seam logic at the UI layer: a hybrid approach where both frameworks could run in the same session during the transition, migrating leaf components and shared services first, then routes module by module, rather than a parallel full rewrite — that mattered specifically because AngularJS's two-way `$scope` binding doesn't map 1:1 onto Angular's component model, so this was a real redesign of component boundaries for each module, not a mechanical port.

On the backend, target-state ASP.NET Core services replaced the .NET Framework monolith's tightly-coupled patterns — built-in DI instead of Unity/Autofac wired through Global.asax, an explicit middleware pipeline instead of IIS's opaque HttpModule chain, and Azure Service Bus replacing Windows Services and file-share polling for background work where the work needed to survive a restart or decouple from the web app's own scaling. I made that call selectively, not as a blanket replacement — some background jobs stayed as simple in-process workers where a queue would have been unjustified infrastructure overhead for what was just a reliable scheduled task.

I didn't personally touch every one of the dozens of applications — the leverage was defining the target-state pattern once and putting an architecture review gate on every wave before it was accepted, so 'clean and testable' and 'observable by default' weren't suggestions, they were conditions of a wave being marked done. That's what let dozens of applications migrate with zero unplanned downtime: the wave-sequencing discipline and the gate criteria were the risk-management tool, not something bolted on after an incident.

The hardest part wasn't the technical migration itself — it was resisting the pull to modernize everything about an app once you were already inside it for one specific piece, because that's exactly what turns a low-risk incremental wave back into a high-risk mini-rewrite. The second-hardest part was the AngularJS component redesign specifically — teams had to relearn unidirectional data flow, not just new syntax, which is why that piece went through architecture review rather than an automated upgrade tool."
...
The same pattern showed up at scale on the Wipro Cloud Migration Programme (Q7) — dozens of applications, each wave owned by a different tech lead, none of them reporting to me. I paired with each wave's tech lead before they started, walking through the target-state pattern on their specific app rather than handing them a spec. One tech lead's first instinct on a particularly entangled legacy app was to lift-and-shift it as-is to save time; instead of overriding that, I walked through what it would cost the programme later — the app would still carry every piece of the technical debt we were trying to remove, just now running in Azure — and let him reach the Strangler Fig decision himself. Show the reasoning once on a real example, then hand off ownership, is the technique either way."
...
The Wipro Cloud Migration Programme (Q7, Q14) needed the same thing at larger scale and higher stakes — multiple wave tech leads, none reporting to me, and an architecture review gate standing in for the authority I didn't have. The gate only works if teams trust it's fair rather than arbitrary, so I made a point of reviewing the *first* wave from every new tech lead personally and in detail, then trusting the pattern for their subsequent waves — earning credibility up front instead of assuming it. Where the standard genuinely didn't fit — a couple of legacy apps had dependencies the standard landing-zone pattern hadn't anticipated — I fed that back into the governance definition and got it amended, rather than forcing every app to awkwardly conform to something that didn't actually apply to it. That's usually the real difference between influence that sticks and a mandate that gets quietly worked around: the standard has to be able to bend when it's actually wrong, or teams stop trusting it."
...
At Wipro, the reusable landing-zone and governance patterns from the Cloud Migration Programme (Q7) are the same idea at platform scale — networking, identity, policy, and monitoring baseline defined once so every wave's target environment inherited it by default, instead of each of the dozens of wave teams independently solving 'how do we stand up a properly governed Azure environment.' The adoption mechanism there wasn't really a sales job either — it was structural: a wave's environment came from the landing-zone template, so using the standard was the path of least resistance, not a separate decision each team had to make."
...
The Wipro Cloud Migration Programme (Q7) needed me in that advisor role continuously, not just once, because I was the one point translating between three audiences who each needed a different version of the same reality. To programme leadership, I reported progress and risk in business terms — waves complete, what's blocking the next one, rollback exposure — not architecture detail they didn't need. To the wave delivery teams, I translated governance requirements into concrete constraints on their specific application, not abstract policy. And when a governance pattern genuinely didn't fit a real app's constraints, I was the one who fed that back into the platform/governance definition and got it amended, rather than making delivery teams awkwardly conform to a pattern that didn't actually apply. Being trusted by leadership mattered less as a single moment and more as a standing role — the person who could be asked 'is this actually on track' and give a straight, non-defensive answer."
...
"The literal AngularJS-to-Angular case is the Wipro Cloud Migration Programme (Q7) — the delivery teams executing the front-end waves had mostly worked in AngularJS for years and hadn't done a real Angular migration before. The syntax wasn't the hard part; the mental-model shift was. AngularJS's two-way `$scope` binding lets a controller be mutated from multiple places with no explicit contract, so I ran the first migrated module as a worked example specifically to show what unidirectional `@Input()`/`@Output()` data flow looks like in practice, not just to describe it — teams don't internalize 'explicit data flow' from a guide, they internalize it from watching a real component get redesigned around it. I also called out the framework-agnostic auth-token handling explicitly, since during the hybrid transition both frameworks needed to agree on one source of truth for the session, and that's an easy thing to get subtly wrong the first time.

Separately, when Capital Access moved from webpack-based Angular to Angular 18 standalone components, a related but distinct skill gap showed up — the standalone-component model removes NgModules, which the team had used as their organizing unit for years. Same technique: ran the migration on one feature module first as a worked example, then had a team member pair with me on the second module and lead the third themselves — the first module shows what it looks like, the second and third are what make it theirs."
